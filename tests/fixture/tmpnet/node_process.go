...
func (p *NodeProcess) Start(w io.Writer) error {
	...
	cmd := exec.Command(p.node.RuntimeConfig.AvalancheGoPath, "--config-file", p.node.getFlagsPath())
	...
	nodeDescription := fmt.Sprintf("node %q", p.node.ID)
	if p.node.IsEphemeral {
		nodeDescription = "ephemeral " + nodeDescription
	}
	nonDefaultNodeDir := filepath.Base(dataDir) != p.node.ID.String()
...
// stop running.
func (p *NodeProcess) Stop(ctx context.Context, waitForStopped bool) error {
	proc, err := p.getProcess()
	if err != nil {
...
func (p *NodeProcess) WaitForStopped(ctx context.Context) error {
	...
	for {
		...
		select {
		case <-ctx.Done():
			return fmt.Errorf("failed to see node process stop %q before timeout: %w", p.node.ID, ctx.Err())
		case <-ticker.C:
		}
	}
}
...
func (p *NodeProcess) waitForProcessContext(ctx context.Context) error {
	...
	for len(p.node.URI) == 0 {
		err := p.readState()
		if err != nil {
			return fmt.Errorf("failed to read process context for node %q: %w", p.node.ID, err)
		}

		select {
		case <-ctx.Done():
			return fmt.Errorf("failed to load process context for node %q before timeout: %w", p.node.ID, ctx.Err())
		case <-ticker.C:
		}
	}
	return nil
