...
// TODO(marun) Document the purpose of the code in this file
// TODO(marun) Indicate that the code for reading and writing a network is in network_config.go
...
var (
	errMissingBootstrapNodes = errors.New("failed to start node due to missing bootstrap node(s)")
)
...
// Instantiate a new bare network intance. Useful for reading an existing network from disk.
...
// Create the network on disk, choosing its network id and generating its genesis in the process.
...
func (n *Network) Stop() error {
	...
	for _, node := range n.Nodes {
		...
		if err := node.WaitForStopped(ctx); err != nil {
			errs = append(errs, fmt.Errorf("failed to wait for node %s to stop: %w", node.ID, err))
		}
	}
...
func (n *Network) Restart(ctx context.Context, w io.Writer) error {
	...
	for _, node := range n.Nodes {
		...
		if err := node.Stop(ctx, true /* waitForStopped */); err != nil {
			return fmt.Errorf("failed to stop node %s: %v", node.ID, err)
		}
		if err := n.StartNode(ctx, w, node); err != nil {
			return fmt.Errorf("failed to start node %s: %v", node.ID, err)
		}
...
// Ensure the provided node has the configuration it needs to start. If the data dir is not
...
func (n *Network) EnsureNodeConfig(node *Node) error {
	...
	if len(n.Dir) > 0 {
		...
		if len(dataDir) == 0 {
			...
			dataDir = filepath.Join(n.Dir, node.ID.String())
...
func (n *Network) GetURIs() []NodeURI {
	// TODO(marun) Maybe limit to initial stakers?
	return GetNodeURIs(n.Nodes)
}

// Retrieve bootstrap IPs and IDs for all nodes except the skipped one (this supports
...
func (n *Network) getBootstrapIPsAndIDs(skippedNode *Node) ([]string, []string, error) {
	...
	for _, node := range nodes {
		...
		if skippedNode != nil && node.ID == skippedNode.ID {
			continue
		}
