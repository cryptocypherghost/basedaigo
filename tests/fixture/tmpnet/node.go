...
// TODO(marun) Indicate that the code for reading and writing a network is in node.go
...
// NodeRuntime defines the functionality required to
...
// Initialize a new node with only the data dir set
...
// Attempt to read a node's configuration from the specified directory.
func ReadNode(dataDir string) (*Node, error) {
	node := NewNode(dataDir)
	return node, node.Read()
}

// Read nodes from the specified network directory.
...
// Retrieve the runtime for the node.
...
func (n *Node) Stop(ctx context.Context, waitForStopped bool) error {
	return n.getRuntime().Stop(ctx, waitForStopped)
}
...
func (n *Node) SetNetworkingConfigDefaults(
	httpPort uint16,
	stakingPort uint16,
	bootstrapIDs []string,
	bootstrapIPs []string,
) {
	n.Flags.SetDefaults(FlagsMap{
...
// BLS signing key.
func (n *Node) GetProofOfPosession() (*signer.ProofOfPossession, error) {
