...
func (n *network) issueTx(tx *txs.Tx) error {
	...
	lastAccepted := n.manager.LastAccepted()
	state, found := n.manager.GetState(lastAccepted)
	if !found {
		n.ctx.Log.Debug("failed retrieving state for last accepted block",
			zap.Stringer("blkID", lastAccepted),
		)
	}
